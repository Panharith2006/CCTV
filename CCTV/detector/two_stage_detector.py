...
class TwoStageDetector:
    ...
    def detect(self, frame, imgsz=416):
        ...
        # Stage 1: persons (suppress YOLO output with verbose=False)
        res_p = self.person_model(frame, imgsz=imgsz, verbose=False)
        for r in res_p:
            boxes = list(r.boxes)
            for b in boxes:
                cls = int(b.cls[0])
        ...
        if self.attr_model is not None and len(detections) > 0:
            for person in detections[:]:
                ...
                # run attr model on the crop (suppress YOLO output)
                res_a = self.attr_model(crop, imgsz=imgsz, verbose=False)
                found = False
                for r in res_a:
                    boxes = list(r.boxes)
                    for b in boxes:
                        ...
                        if conf < self.conf_attr:
                            continue
                        lname = name.lower()
                        # Only count positive detections: with-mask, with-helmet, mask (but NOT no-mask, no-helmet)
                        if lname in ['with-mask', 'mask', 'wearing-mask', 'has-mask']:
                            label = 'mask'
                        elif lname in ['with-helmet', 'helmet', 'hardhat', 'wearing-helmet', 'has-helmet']:
                            label = 'helmet'
                        else:
                            # Skip negative detections (no-mask, no-helmet) and unknown classes
                            continue
                ...
                if not found:
                    ...
                    if fx2 > fx1 and fy2 > fy1:
                        ...
                        res_af = self.attr_model(full_crop, imgsz=imgsz, verbose=False)
                        for r2 in res_af:
                            ...
                            for b2 in boxes2:
                                ...
                                lname2 = name2.lower()
                                # Only count positive detections: with-mask, with-helmet, mask (but NOT no-mask, no-helmet)
                                if lname2 in ['with-mask', 'mask', 'wearing-mask', 'has-mask']:
                                    label2 = 'mask'
                                elif lname2 in ['with-helmet', 'helmet', 'hardhat', 'wearing-helmet', 'has-helmet']:
                                    label2 = 'helmet'
                                else:
                                    # Skip negative detections (no-mask, no-helmet) and unknown classes
                                    continue
